...
export function LoginPage() {
    ...
    const handleLogin = async (e: FormEvent) => {
        ...
        try {
            ...
            // Un instante de cierre visual antes de saltar al dashboard —
            // sin esto el login se siente como un corte seco, no como un logro.
            setLoading(false);
            setSuccess(true);
            setTimeout(() => navigate('/dashboard', { viewTransition: true }), 450);
        } catch (err) {
    ...
    // Delay de entrada en cascada — 60ms entre cada elemento, tope de ~5
    // pasos para que la bienvenida no se sienta lenta (skill emil-design-eng).
    const enterDelay = (step: number) => ({ '--enter-delay': `${step * 60}ms` } as CSSProperties);
    ...
    const renderForm = () => (
        <form onSubmit={handleLogin} className="space-y-5">
            <div>
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        required
                        autoFocus
                        className="block w-full pl-11 pr-3 py-3 bg-card text-cb-text-primary border border-cb-border rounded-xl focus:ring-4 focus:ring-primary/12 focus:border-primary transition-[box-shadow,border-color] duration-200 ease-out outline-none text-sm font-medium placeholder:text-cb-neutral/50"
                    />
                </div>
            </div>
            ...
            <div>
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        required
                        className="block w-full pl-11 pr-11 py-3 bg-card text-cb-text-primary border border-cb-border rounded-xl focus:ring-4 focus:ring-primary/12 focus:border-primary transition-[box-shadow,border-color] duration-200 ease-out outline-none text-sm font-medium placeholder:text-cb-neutral/50"
                    />
                    <button
                        type="button"
            ...
            <button
                ...
                className={`w-full h-12 flex items-center justify-center gap-2 text-primary-foreground rounded-xl font-bold text-sm tracking-wide shadow-lg transition-[background-color,transform] duration-200 ease-out active:scale-[0.98] disabled:cursor-not-allowed cursor-pointer ${
                    success ? 'bg-emerald-500 shadow-emerald-500/20' : 'bg-primary shadow-primary/20 hover:bg-primary/90 disabled:opacity-60'
                }`}
            >
                {success ? (
                    <Check className="w-5 h-5 pop-in" />
                ) : loading ? (
        ...
        <div className="min-h-dvh flex flex-col md:flex-row bg-[#F7F8FA]">
            ...
            <div className="flex flex-col md:hidden min-h-dvh w-full">
                <div className="relative bg-primary overflow-hidden shrink-0 pb-12 pt-10 min-h-[38dvh]">
                    ...
                    <div className="relative z-10 flex flex-col items-center text-center px-6">
                        <div className="w-16 h-16 rounded-2xl bg-white flex items-center justify-center shadow-xl mb-5 enter-fade-up" style={enterDelay(0)}>
                            <span className="text-primary font-black text-2xl tracking-tighter">S</span>
                        </div>
                        <h1 className="text-white text-2xl font-bold tracking-tight enter-fade-up" style={enterDelay(1)}>Grupo Sole</h1>
                        <p className="text-white/75 text-sm mt-1.5 max-w-[240px] enter-fade-up" style={enterDelay(2)}>{t('auth.subtitle')}</p>
                    </div>
                ...
                <div className="flex-1 flex flex-col justify-center px-6 py-8">
                    <div className="max-w-sm mx-auto w-full enter-fade-up" style={enterDelay(3)}>{renderForm()}</div>
                </div>
            </div>
            ...
            <div className="hidden md:flex md:flex-row w-full">
                <div className="hidden md:flex flex-col justify-between w-1/2 bg-primary text-white p-12 lg:p-16 relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex items-center gap-3 enter-fade-up" style={enterDelay(0)}>
                    ...
                    <div className="relative z-10 max-w-md enter-fade-up" style={enterDelay(1)}>
                    ...
                    <div className="relative z-10 text-xs text-white/50 font-medium enter-fade-up" style={enterDelay(2)}>
                ...
                <div className="flex-1 flex flex-col justify-center items-center p-8 lg:p-12 bg-[#F7F8FA] relative">
                    <div className="w-full max-w-[380px] space-y-8">
                        <div className="text-center md:text-left enter-fade-up" style={enterDelay(1)}>
                        ...
                        <div className="bg-card border border-cb-border rounded-2xl shadow-xl shadow-slate-200/40 p-8 enter-fade-up" style={enterDelay(2)}>
                            {renderForm()}
                        </div>
                    </div>
                </div>
